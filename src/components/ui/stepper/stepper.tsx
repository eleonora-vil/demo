...
const StepperProvider = ({ value, children }: { value: StepperProps; children: React.ReactNode }) => {
...
export const Stepper = React.forwardRef<HTMLDivElement, StepperProps & React.HTMLAttributes<HTMLDivElement>>(
  (
    {
      initialStep,
      steps,
    ...
    return (
      ...
      >
        <div ref={ref} className={cn('space-y-4', className)} {...props}>
          <div className={cn('flex w-full flex-1 justify-between gap-2 text-center', orientation === 'vertical' ? 'flex-col' : 'flex-row')}>{items}</div>
...
const HorizontalContent = ({ children }: { children?: React.ReactNode }) => {
  ...
  const activeStepperItem = React.Children.toArray(children)[activeStep] as React.ReactElement;
...
>(({ step, children, className, onStepperItemClick, ...props }, ref) => {
  const { activeStep, setStep, steps, orientation, labelOrientation, scrollTracking, variant, status, isClickable } = useStepper();
  ...
  const content = React.Children.toArray(children).filter((child) => React.isValidElement(child) && child.type !== StepperFooter);

  const footer = React.Children.toArray(children).filter((child) => React.isValidElement(child) && child.type === StepperFooter)[0] as React.ReactElement;
  ...
  return (
    ...
    >
      <div
        className={cn('flex items-center gap-3', isVerticalLabel && !isVertical && 'flex-col gap-2')}
        ref={(node) => {
          if (scrollTracking) {
            node?.scrollIntoView({
              behavior: 'smooth',
      ...
      >
        ...
        <div className={cn('w-max text-start', isVerticalLabel && !isVertical && 'text-center')}>
          <p className="text-sm text-slate-900 dark:text-slate-100">{steps[step].label}</p>
          {steps[step].description && <p className="text-xs text-slate-500 dark:text-slate-400">{steps[step].description}</p>}
        </div>
      </div>
      <div className="flex w-full gap-8">
        {!isLastStep && (
          <Separator
            className={cn('stepper-item-separator', isVertical ? 'ms-6 flex h-auto min-h-[2rem] w-[1px] border-l-2' : 'h-[2px] flex-1', isCompleted && 'bg-green-700')}
...
export const StepperFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ children, ...props }, ref) => {
  return (
    <div ref={ref} {...props}>
      {children}
    </div>
