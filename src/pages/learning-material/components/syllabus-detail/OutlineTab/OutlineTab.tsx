...
import NoLearningObjRow from './NoLearningObjRow';
          ...
          return (
            <AccordionItem value={index.toString()} key={index}>
              ...
              <AccordionContent className="mb-2 rounded-b-md flex flex-col gap-2 py-2 px-5">
                {createLearningObjects?.length > 0 ? (
                  createLearningObjects.map((details: any, index: number) => (
                    <LearningObjRow learningObjs={details} key={`learning-objective-${index}`} />
                  ))
                ) : (
                  <NoLearningObjRow />
                )}
