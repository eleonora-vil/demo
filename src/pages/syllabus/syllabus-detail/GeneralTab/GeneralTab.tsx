  ...
  return (
    <div>
      <div className=" w-full flex gap-6 justify-start pt-2 mb-5">
        ...
        <ContentCard title="Technical Requirement(s)" icon={<NavigationIcons icon="settings" />} className="flex-[3] break-all w-2/3 ">
...
function removeDuplicateObjects<T>(array: T[], key: keyof T): T[] {
  const uniqueObjects = new Map<any, T>();

  for (const item of array) {
    const keyValue = item[key];
    uniqueObjects.set(keyValue, item);
  }
...
function extractOutputStandards(obj: any) {
  ...
  return removeDuplicateObjects(outputStandards, 'outputStandardId');
}
